...
function drawTowers(){
  ...
  tower1.style.top=offsettop+offsethoriz+"px";
  tower2.style.left=offsetleft+towerwidth+offsettower+"px";
  tower2.style.top=offsettop+offsethoriz+"px";
  tower3.style.left=offsetleft+(towerwidth+offsettower)*2+"px";
  tower3.style.top=offsettop+offsethoriz+"px";
...
function newGame(obj){
    if (movectr>0 && !gameOver && !stop){
        if (confirm("Ván game hiện tại sẽ dừng lại, bạn có muốn tiếp tục?", "Tháp Hà Nội")){
            initVars();
...
function initializeDrag(disk,e){
  if (!e) e=event;
  if (stop){
    alert("Bạn không thể tiếp tục ván game.\nChọn 'Chơi lại' hoặc chọn lại số lượng đĩa để tiếp tục game.", "Tháp Hà Nội");
    return;
}
indexFr = indexTo;
...
function dropDisk(disk){
  ...
  if (indexFr==indexTo){
    ...
    if (gameStatus) {
        ...
        if (movectr==minmove) msg="\nChúc mừng! Bạn đã hoàn thành trong "+minmove+" lần di chuyển."
            else if (movectr>minmove) msg="\nBạn có thể làm tốt hơn vậy mà!"
                else msg="";
            alert("Kết thúc !!!"+msg, "Tháp Hà Nội");
            gameOver=true;
        }
        return;
    }      
...
function solve(btn){
    if (btn.name=="btnSolve"){
        if (movectr>0 && !gameOver && !stop)
            if (!confirm("Ván game hiện tại sẽ dừng lại, bạn có muốn tiếp tục?")) return;
        btn.value="Dừng";
        initVars();
        stop=false;
        demo=true;
...
function moveDisk(){
    ...
    if (movectr<parseInt(frm.minmove.value)) t=window.setTimeout("moveDisk()",delay);
        ...
        alert("Bạn có thể thực hiện trong "+movectr+" bước di chuyển không?", "Tháp Hà Nội");
        gameOver=true;
        stop=false;
        frm.btnSolve.value="Đáp án";
        frm.btnIns.disabled=false;
        frm.btnRes.disabled=false;
    }
}
...
function displayIns(){
    var msg="Cố gắng di chuyển tất cả các đĩa từ tháp 1 sang tháp 3.\n";
    msg+="Mỗi lần bạn chỉ được phép di chuyển 1 đĩa.\n";
    msg+="Bạn không được phép đặt đĩa lớn ở trên đĩa nhỏ hơn.";
    alert(msg, "Tháp Hà Nội");
}